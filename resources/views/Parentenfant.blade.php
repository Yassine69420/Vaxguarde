...
<body>
    <div class="container">
        <div style="flex-direction: row;" class="container bord card body-card p-3 my-4 d-flex justify-content-between align-items-center rounded">
            <img class="logo" height="60" src="{{ asset('assets\images\logot.png') }}" alt="" srcset="">
        ...
        <div class="container mt-4">
            @php
                ...
                function calculateVaccinationDate($birthDate, $requiredAge) {
                    ...
                    return $vaccinationDate->format('F Y'); // Format to display month and year only
                }
            @endphp
            ...
            <table class="table mt-4">
                ...
                @foreach ($vaccinationStatuses as $vaccine => $details)
                    ...
                    <tr>
                        ...
                        <td>
                            {{ $details['age'] }} mois 
                            @if(!$details['status'] && $age < $details['age'])
                                <span class="date">(doit être pris le {{ $vaccinationDate }})</span>
                            @endif
                        </td>
                        <td class="{{ $details['status'] ? 'text-success' : ($age < $details['age'] ? 'text-secondary' : 'text-danger') }}">
                            {{ $details['status'] ? 'déjà Vacciné' : ($age < $details['age'] ? 'en attendant' : 'Obligatoire') }}
                        </td>
                    </tr>
                @endforeach
            </table>
